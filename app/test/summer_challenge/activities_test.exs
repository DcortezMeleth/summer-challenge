defmodule SummerChallenge.ActivitiesTest do
  ...
  describe "get_user_activities/2" do
    ...
    test "returns activities for a user and challenge", %{user: user, challenge: challenge} do
      ...
      activity1 = Repo.insert!(%Activity{
        user_id: user.id,
      ...
      activity2 = Repo.insert!(%Activity{
        user_id: user.id,
        challenge_id: challenge.id,
        strava_id: 2,
        sport_type: "Ride",
    ...
    test "only returns activities for the specified challenge", %{user: user, challenge: challenge} do
      ...
      _activity1 = Repo.insert!(%Activity{
        user_id: user.id,
        challenge_id: challenge.id,
        strava_id: 1,
        sport_type: "Run",
      ...
      _activity2 = Repo.insert!(%Activity{
        user_id: user.id,
    ...
    test "includes excluded activities in results", %{user: user, challenge: challenge} do
      _excluded_activity = Repo.insert!(%Activity{
        user_id: user.id,
        challenge_id: challenge.id,
        strava_id: 1,
        sport_type: "Run",
  ...
  describe "toggle_activity_exclusion/2" do
    ...
    test "returns error for non-existent activity", %{user: user} do
      ...
      assert {:error, :not_found} = Activities.toggle_activity_exclusion(fake_activity_id, user.id)
    end

    test "returns error when user tries to toggle another user's activity", %{other_user: other_user, activity: activity} do
      assert {:error, :unauthorized} = Activities.toggle_activity_exclusion(activity.id, other_user.id)

defmodule SummerChallengeWeb.CoreComponents do
  ...
  def icon(%{name: "hero-flag"} = assigns) do
    ~H"""
    <svg class={@class} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
  ...
  Enhanced with icons, hover effects, and active state indicators.
  """
  ...
  def auth_section(assigns) do
    ...
    <div class="flex items-center justify-between py-4">
      <%= if @current_scope.authenticated? do %>
        ...
        <!-- Navigation menu with icons and hover effects -->
        <nav class="flex items-center gap-1.5" aria-label="Main navigation">
          ...
          <!-- Divider -->
          <div class="h-6 w-px bg-brand-700 mx-1"></div>
          
          <!-- Sign out button -->
          <.link
            href="/auth/logout"
            method="delete"
            class="group relative flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-brand-200 hover:text-white transition-all duration-200 hover:bg-brand-800/50"
          >
            <.icon name="hero-arrow-right-on-rectangle" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" />
            <span>Sign out</span>
          </.link>
        </nav>
      <% else %>
  ...
  # Navigation item component with icon, hover effect, and active state.
  ...
  defp nav_item(assigns) do
    ...
    <.link
      ...
      class={[
        ...
        if(@is_active,
          do: "text-white bg-brand-700/70 shadow-lg",
          else: "text-brand-200 hover:text-white hover:bg-brand-800/50"
        )
      ]}
    >
      ...
      <.icon
        name={@icon}
        class={"w-4 h-4 transition-transform duration-200 #{unless @is_active, do: "group-hover:scale-110", else: ""}"}
      />
      ...
      <div
        :if={!@is_active}
        class="absolute inset-0 rounded-lg bg-brand-400/0 group-hover:bg-brand-400/5 transition-colors duration-200 -z-10"
      >
      </div>
    </.link>
    """
